...
const config = {
  ...
  preprocessingRules: [
    { // [[ include: path1/.../file.md ]]
      ...
      replace(raw, includePath) {
        const processingFile = this.processingFile
        if (includePath === '../../liste-erreurs/erreurs-par-api/err-con3.md') {
          console.log('processingFile :', processingFile)
        }
        logger.enabledLevels.trace && log.trace(
          `processing include directive "${includePath}" in ${processingFile}`
        )
        const srcDir = this.getConfig('srcDir')
        if (includePath.indexOf('/') !== 0) {
          includePath = path.resolve(path.dirname(path.join(srcDir, processingFile)), includePath)
        } else {
      ...
      pattern: /[\[][{][\s]*([\w]+)[\s]*[}][\]]/g,
      replace(raw, content) {
        const docName = _.first(this.processingFile.split('/'))
        const srcDir = this.getConfig('srcDir')
        let text
        try {
          const config = YAML.parse(fs.readFileSync(path.join(srcDir, docName, 'config.yml'), 'utf8'))
          text = _.get(config, content)
        } catch (err) {
          // Do not remove
        }
        if (!text) {
          text = `<span class="error">"${content}" not defined</span>`
        }
        return text
      }
    }
  ...
  renderingRules: {
    ...
    code(orig, code, lang, escaped) {
      const words = lang && lang.split('-')
      return _.first(words) === 'mermaid'
        ? `<div class="${words.join(' ')}">${code}\n</div>`
        : orig(code, lang, escaped)
    }
  },
