...
const config = {
  ...
  // Temporary directory containing doc files meta datas
  metaDir: path.join(homeDir, `.${pkg.name}`, 'meta'),
  ...
  themes: helper.fs.readdirSync(path.join(baseDir, 'themes')).reduce((acc, dir) => {
    ...
    { // {{{{{{ websequence diagram }}}}}}
      ...
      replace(raw, includePath) {
        ...
        if (includePath.indexOf('/') !== 0) {
          includePath = path.resolve(path.join(srcDir, this.processingFile), includePath);
        } else {
          includePath = path.join(srcDir, includePath);
        }
        let done = false;
        let html;
        this.processDoc(path.relative(srcDir, includePath), (err, meta) => {
          if (err) {
