...
class ServeDown {
  ...
  constructor(config) {
    this.config = config;
    this.locals = {};
    logger.enabledLevels.debug && log.debug(`${pkg.name} instance created`);
  }
  ...
  init(config) {
    ...
    if (!fs.existsSync(this.locals.config.workingDir)) {
      logger.enabledLevels.debug && log.debug(`creating working dir`);
  ...
  updateGitRepo(repos, contentDir, cb) {
    const executeCmd = Promise.promisify(childProcess.exec, {multiArgs: true});
      ...
      return fs.statAsync(repoDir)
        .then(stat => {
          if (stat.isDirectory()) {
            ...
            return executeCmd('git pull', {cwd: repoDir});
          }
        ...
        .spread((stdout, stderr) => {
          if (stdout && logger.enabledLevels.debug) {
            logger.enabledLevels.debug && log.debug(stdout);
          }
          if (stderr && logger.enabledLevels.warn) {
  ...
  scanMdFiles(baseDir, opt = {}, cb) {
    ...
    return p.reduce((found, file) => fs
      ...
      .then(stats => {
        ...
        logger.enabledLevels.debug && log.debug(`found file : ${file}`);
  ...
  computeDoc(file, cb) {
    ...
    return fs.readFileAsync(path.join(this.locals.config.workingDir, file), 'utf8')
      ...
      .then(content => {
        ...
        const uri = `/${filename}`;
        logger.enabledLevels.debug && log.debug(`register route ${uri} with file ${file}`);
  ...
  compute(config, cb) {
    ...
    return Promise.resolve()
      ...
      .then(() => this
        .scanMdFiles(this.locals.config.workingDir, {include: this.locals.config.mdExtPattern})
