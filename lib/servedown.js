...
class ServeDown {
  ...
  init(config) {
    ...
    try {
      config = typeof this.config === 'string'
    ...
    this.locals.config = _.merge({}, defaultConfig, config);
    logger.enabledLevels.debug && log.debug(`${pkg.name} instance initialized with :`, JSON.stringify(this.config));
    ...
    return servedown.compute()
      .then(() => {
        ...
        if (socket) {
          ...
          return new Promise(resolve => app.listen(socket, resolve))
            .then(() => {
              logger.enabledLevels.info && log.info('servedown is listening to socket %s', socket);
            });
        }
        let port = servedown.getConfig('server.port');
        if (port) {
          port = parseInt(port);
        }
        const host = servedown.getConfig('server.host');
        return new Promise(resolve => (host ? app.listen(port, host, resolve) : app.listen(port, resolve)))
          .then(() => {
            logger.enabledLevels.info && log.info('servedown is listening to %s',
