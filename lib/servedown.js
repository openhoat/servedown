...
class ServeDown {
  ...
  init(config) {
    ...
    this.locals.config = Object.assign({}, defaultConfig, config);
    ...
    return servedown.compute()
      .then(() => {
        ...
        if (socket) {
          ...
          return new Promise(
            resolve => {
              this.server = app.listen(socket, resolve);
            })
            .then(() => {
        ...
        return new Promise(
          resolve => {
            this.server = (host ? app.listen(port, host, resolve) : app.listen(port, resolve));
          })
          .then(() => {
