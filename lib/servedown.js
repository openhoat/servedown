...
class ServeDown {
  ...
  init(config) {
    ...
    _.forIn(this.getConfig('renderingRules', {}), (value, key) => {
      if (typeof value === 'function') {
        value = value.bind(this, renderer[key] ? renderer[key].bind(renderer) : 'toto')
      }
      renderer[key] = value
    })
  ...
  processDoc(file, cb) {
    ...
    return Promise.resolve()
      ...
      .then(mdContent => {
        ...
        const uri = repo && repo.baseDir ? `/${filename}`.replace(`/${repo.baseDir}`, '') : `/${filename}`
  ...
  expressMwDoc(req, res, next) {
    ...
    return this.getDocMeta(req.path)
      .then(meta => {
        if (!meta) {
          const repo = helper.findRepo(res.locals.context, this.getConfig('repos'))
          const reqPath = repo && repo.baseDir ?
            ['', repo.name, repo.baseDir].concat(_.tail(_.tail(req.path.split('/')))).join('/') :
            req.path
          return Promise.fromCallback(this.cache.keys.bind(this.cache))
            .then(keys => {
              if (keys && keys.includes(reqPath + '/')) {
                return res.redirect(req.path + '/')
              }
              return Promise.fromCallback(res.sendFile.bind(res, reqPath, {root: this.getConfig('srcDir')}))
            })
        }
          ...
          .then(listenOpts => Promise
            ...
            .then(() => {
              this.server.on('connection', socket => {
                ...
                logger.enabledLevels.trace && log.trace('new socket (#%s)', socketId)
                socket.once('close', () => {
                  logger.enabledLevels.trace && log.trace('socket #%s closed', socketId)
                  this.sockets.splice(socketId - 1, 1)
                })
              })
        ...
        return new Promise(resolve => {
          ...
          if (this.sockets.length) {
            this.sockets.forEach((socket, index) => {
              logger.enabledLevels.trace && log.trace('destroying socket #%s', index + 1)
              socket.destroy()
            })
          }
        })
...
if (!module.parent || module.parent.filename === path.normalize(path.join(__dirname, '..', 'bin', pkg.name))) {
  ServeDown.start(path.join(helper.homeDir, `.${pkg.name}.yml`))
    .then(servedown => {
      ...
      const sigintHandler = () => {
        log.warn('received signal SIGINT')
        exit()
      }
